...
export default function ArtifactViewerPage({
  ...
  const [activeType, setActiveType] = useState<
    "executive_brief" | "advisory" | "presentation"
  >("executive_brief");

  const currentArtifact =
    artifacts.find((a) => a.id === activeArtifactId) ||
    artifacts.find((a) => a.type === activeType);
  ...
  const renderContent = (content: string, sectionId: string) => {
    if (
      currentArtifact?.type === "executive_brief" &&
      content.includes("180 servers")
    ) {
          ...
          <span
            ...
            onClick={() => {
              ...
              openProvenance({
                ...
                sourceText:
                  "...a total of 18 servers distributed across three network segments were confirmed to be affected by the intrusion...",
                dnaVersion: "1.0",
                confidence: 0.96,
              });
            }}
    ...
    return (
      <p className="text-nexus-text-secondary leading-relaxed">{content}</p>
    );
  };
    ...
    <div className="h-screen flex flex-col bg-nexus-bg overflow-hidden">
      ...
      <div className="h-16 border-b border-nexus-border bg-nexus-panel flex items-center justify-between px-6 shrink-0">
        <div className="flex items-center gap-4">
          <Link
            href="/transform/src-001/progress"
            className="text-sm font-medium text-nexus-text-secondary hover:text-white transition-colors"
          >
            Transformation
          </Link>
        ...
        <div className="flex bg-nexus-card p-1 rounded-lg border border-nexus-border">
          <button
            onClick={() => {
              setActiveType("executive_brief");
              setActiveArtifact(
                allForSource.find((a) => a.type === "executive_brief")?.id ||
                  "",
              );
            }}
            className={cn(
              "px-4 py-1.5 text-sm font-medium rounded-md flex items-center gap-2",
              activeType === "executive_brief"
                ? "bg-nexus-cyan text-[#0a0f1e]"
          ...
          <button
            onClick={() => {
              setActiveType("advisory");
              setActiveArtifact(
                allForSource.find((a) => a.type === "advisory")?.id || "",
              );
            }}
            className={cn(
              "px-4 py-1.5 text-sm font-medium rounded-md flex items-center gap-2",
          ...
          <button
            onClick={() => {
              setActiveType("presentation");
              setActiveArtifact(
                allForSource.find((a) => a.type === "presentation")?.id || "",
              );
            }}
            className={cn(
              "px-4 py-1.5 text-sm font-medium rounded-md flex items-center gap-2",
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-64 border-r border-nexus-border bg-nexus-panel/50 flex flex-col hidden md:flex shrink-0">
          <div className="p-4 border-b border-nexus-border font-semibold text-white text-sm">
            Document Outline
          </div>
          <div className="p-2 overflow-y-auto flex-1 space-y-1">
            {currentArtifact.sections.map((section, idx) => (
              ...
              >
                ...
                {currentArtifact.validationIssues.some(
                  (vi) => vi.sectionId === section.id,
                ) && (
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-8 bg-nexus-panel p-4 rounded-xl border border-nexus-border">
              <div>
                ...
                <div className="flex items-center gap-3 mt-1 text-xs">
                  <span className="text-nexus-text-secondary">
                    v{currentArtifact.dnaVersion}
                  </span>
                  <span
                    className={cn(
                      "px-2 py-0.5 rounded uppercase font-semibold",
                      currentArtifact.status === "approved"
              ...
              <div className="flex gap-2">
                <button
                  className="p-2 bg-nexus-card border border-nexus-border rounded hover:bg-nexus-card/80 text-nexus-text-secondary hover:text-white transition-colors"
                  title="Edit"
                >
            ...
            <div className="bg-white text-black p-10 rounded-xl shadow-2xl min-h-[800px] mb-12">
              <div className="border-b-2 border-gray-200 pb-4 mb-8">
                ...
                <p className="text-gray-500 text-sm mt-2 font-mono">
                  Generated:{" "}
                  {new Date(currentArtifact.generatedAt).toLocaleString()}
                </p>
              </div>
            ...
            >
              ...
              <div className="p-6 overflow-y-auto flex-1 space-y-6">
                ...
                <div className="pt-4 border-t border-nexus-border flex gap-3">
                  <Link
                    href={`/validation/src-001`}
                    className="flex-1 text-center py-2 bg-nexus-cyan text-[#0a0f1e] font-medium rounded-lg text-sm hover:bg-nexus-cyan/90 transition-colors"
                  >
