...
import { 
  CheckCircle, AlertTriangle, ShieldAlert, Check, X,
  FileText, ExternalLink, Activity
} from 'lucide-react';
import Link from 'next/link';

export default function ValidationCenterPage({ params }: { params: { id: string } }) {
  const { mockValidationSummary, artifacts, resolveValidationIssue, openProvenance, provenanceOpen, activeProvenanceLink, closeProvenance } = useNexusStore((state) => ({
    mockValidationSummary: { // manual inject for demo as it's not exported from state directly
      totalClaims: 31, verified: 28, conflicts: 1, unsupported: 1, uncertain: 1,
      numbersChecked: 12, datesChecked: 7, entitiesChecked: 11
    },
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'conflicts' | 'verified'>('conflicts');

  // Find the critical issue
  const executiveBrief = artifacts.find(a => a.type === 'executive_brief');
  const criticalIssue = executiveBrief?.validationIssues.find(vi => vi.id === 'vi-001');
        ...
        <div className="flex-1 overflow-y-auto p-8 relative">
          <div className="max-w-5xl mx-auto space-y-8">
            ...
            <div>
              ...
              <p className="text-nexus-text-secondary mt-2">Source: <span className="text-nexus-cyan font-mono">Cybersecurity_Incident_Report_Sept2026.pdf</span></p>
            </div>
            ...
            <div className="grid grid-cols-5 gap-4">
              <div className="bg-nexus-panel border border-nexus-border rounded-xl p-4 flex flex-col items-center justify-center">
                ...
                <span className="text-xs text-nexus-text-secondary uppercase tracking-wider">Claims Verified</span>
              </div>
              <div className="bg-nexus-red/10 border border-nexus-red/30 rounded-xl p-4 flex flex-col items-center justify-center relative overflow-hidden">
                ...
                <span className="text-2xl font-bold text-nexus-red relative z-10">1</span>
                <span className="text-xs text-nexus-red uppercase tracking-wider relative z-10">Conflict Detected</span>
              </div>
              <div className="bg-nexus-panel border border-nexus-border rounded-xl p-4 flex flex-col items-center justify-center">
                <Activity className="w-6 h-6 text-nexus-cyan mb-2" />
                <span className="text-2xl font-bold text-white">12</span>
                <span className="text-xs text-nexus-text-secondary uppercase tracking-wider">Numbers Checked</span>
              </div>
              <div className="bg-nexus-panel border border-nexus-border rounded-xl p-4 flex flex-col items-center justify-center">
                <Activity className="w-6 h-6 text-nexus-cyan mb-2" />
                <span className="text-2xl font-bold text-white">7</span>
                <span className="text-xs text-nexus-text-secondary uppercase tracking-wider">Dates Checked</span>
              </div>
              <div className="bg-nexus-panel border border-nexus-border rounded-xl p-4 flex flex-col items-center justify-center">
                <Activity className="w-6 h-6 text-nexus-cyan mb-2" />
                <span className="text-2xl font-bold text-white">11</span>
                <span className="text-xs text-nexus-text-secondary uppercase tracking-wider">Entities Checked</span>
              </div>
            </div>
            ...
            {activeTab === 'conflicts' && criticalIssue && criticalIssue.status === 'open' && (
              <motion.div 
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h2 className="text-lg font-bold text-white">FACTUAL CONFLICT — CRITICAL</h2>
                  </div>
                  <span className="px-3 py-1 bg-nexus-panel border border-nexus-border rounded text-xs text-nexus-text-secondary font-mono">Fact ID: {criticalIssue.factId}</span>
                </div>
                
                <div className="grid grid-cols-2 gap-6 mb-6">
                  <div className="bg-nexus-red/5 p-4 rounded-lg border border-nexus-red/10">
                    <div className="text-xs font-semibold text-nexus-red mb-2 uppercase">Generated Artifact Output</div>
                    <p className="text-sm text-white font-medium">"...<span className="text-nexus-red bg-nexus-red/20 px-1 rounded">180 servers</span> were compromised..."</p>
                  </div>
                  <div className="bg-nexus-green/5 p-4 rounded-lg border border-nexus-green/10">
                    <div className="text-xs font-semibold text-nexus-green mb-2 uppercase">Source Document Truth</div>
                    <p className="text-sm text-white font-medium">"...<span className="text-nexus-green bg-nexus-green/20 px-1 rounded">18 servers</span> distributed across three network segments..."</p>
                    <div className="mt-2 text-xs text-nexus-text-secondary">Source: {criticalIssue.sourceFile}, Page {criticalIssue.sourcePage}</div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 pt-4 border-t border-nexus-border">
                  ...
                  <button 
                    onClick={() => resolveValidationIssue(criticalIssue.artifactId, criticalIssue.id, 'ignore')}
            ...
            {activeTab === 'verified' && (
              <div className="bg-nexus-panel border border-nexus-border rounded-xl p-6 space-y-4">
                <h3 className="text-lg font-semibold text-white mb-4">Verified Claims</h3>
                {[
                  ...
                  <div key={i} className="flex items-start gap-3 p-4 bg-nexus-card rounded-lg border border-nexus-border">
                    <CheckCircle className="w-5 h-5 text-nexus-green shrink-0 mt-0.5" />
                    <div>
                      <p className="text-sm font-medium text-white">{claim}</p>
                      <p className="text-xs text-nexus-text-secondary mt-1">Cross-referenced with source data. Confidence: {94 - i}%</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-nexus-panel border border-nexus-border rounded-xl p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-nexus-card border border-nexus-border rounded-lg flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-sm font-medium text-white">SHA-256 Verified</div>
                    <div className="text-xs text-nexus-text-secondary">Integrity hash matches original upload</div>
                  </div>
                </div>
                <div className="p-4 bg-nexus-amber/10 border border-nexus-amber/30 rounded-lg flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-sm font-medium text-nexus-amber">2 Sensitive Items Flagged</div>
                    <div className="text-xs text-nexus-amber/80">Internal IPs and emails detected</div>
                  </div>
                </div>
              </div>
            </div>
